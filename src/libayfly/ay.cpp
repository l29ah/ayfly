...
unsigned long ay::ayProcess(unsigned char *stream, unsigned long len)
{
    ...
    if(src_remaining > 0)
    {
        ...
        {
            stream16 [i] = ay_temp_buffer_out [i];
        }
    ...
    {        
        ...
        while(i < AY_TEMP_BUFFER_SIZE)
        {            
            if(flt_state > flt_state_limit)
            {
                flt_state -= flt_state_limit;
                ay_temp_buffer_out [k] = ay_temp_buffer_in [i];
                ay_temp_buffer_out [k + 1] = ay_temp_buffer_in [i + 1];
                k += 2;                
            }
        ...
        unsigned long remaining = k > to_process ? to_process : k;
